...
<html lang="en">
    ...
    <body style="background-image: url('../img/electricitybill.jpg'); background-repeat: no-repeat; background-size: cover;">
        <header style="flex-direction: row; justify-content: space-around;">
            <img class="icon" src="../img/icon.jpeg" style="left: 0%">
            <h1 id="electricity">electricity bill</h1>
            ...
            <button class="clkbtn" onclick="window.location.href='../php/logoutsession.php'" style="color:black">Logout</button>
        </header>
            ...
            <form method='post' class="login-box" style="gap: 3%;">
                ...
                if($_SESSION['admin'] != TRUE){
                    ...
                    try{
                        ...
                        $sql = "SELECT * from electricity where aadharno = '$aadhar' order by id desc";
                    ...
                    if(array_key_exists('payment', $_POST)){
                        try{
                            ...
                            $sql = "UPDATE electricity SET paid = TRUE where houseno = '$house' and State_ = '$state' and month_ = '$month'";
                            mysqli_query($conn, $sql);
                            $sql = "UPDATE electricity SET paymentdate = CURDATE() where houseno = '$house' and State_ = '$state' and month_ = '$month'";
                            mysqli_query($conn, $sql);
                            $flag = TRUE;
                        }catch(exception $e){
                            echo $sql . "<br>" . $e->getMessage();
                    ...
                    if(array_key_exists('payment', $_POST)){
                        ...
                        try{
                            ...
                            $sql = "INSERT INTO electricity(aadharno, houseno, State_, amount, month_, billdate, paid) VALUES ('$aadhar', '$house', '$state', '$amount', '$month', CURDATE(), 'FALSE')";
                ...
                <input type="text" name="house" class="email ele" placeholder="Consumer no" required>
                <input type="text" name="state" class="email ele" placeholder="State" required>
                <input type="text" name="amount" class="email ele" placeholder="Amount" required>
                <?php
                if($flag == TRUE){
                    if($_SESSION['admin'] == TRUE){
                ...
                ?>
                <input type="submit" name="payment" class="clkbtn" value="Payment">
            </form>
        </div>
    </body>
</html>